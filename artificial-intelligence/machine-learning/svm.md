...
对SVC来说，SVM的任务是找到一条分界线（*超平面，hyperplane*），使得它到两边的margin都最大。下图中，每一个圆形与正方形都代表一个样本数据，在Maximum Margin上的这些数据点就是*支持向量（Support Vector）*。具体来说就是最终分类器表达式中只含有这些支持向量的信息，与其他的数据点无关。
...
> Note：
>  - 超平面
>> 在数学中，超平面是$n$维度欧里几得空间中余维度等于$1$的线性子空间。这是平面中直线、空间中平面的推广。[8][8]
> - VC维
